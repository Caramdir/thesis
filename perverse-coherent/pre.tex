...
Since $\sheafHom$ commutes with the forgetful functor, if $\dualizingCx$ is an equivariant dualizing complex, then the sheaf $\Forget(\dualizingCx) ∈ \dcohb X$ is a (non-equivariant) dualizing complex.
...
\begin{Thm}[{\cite[Theorem~3.10]{ArinkinBezrukavnikov:2010:PerverseCoherentSheaves}}]
    ...
    If $p$ is monotone and comonotone, then the pair
    \[
        \bigl(\dpc{≤0}{X},\, \dpc{≤0}{X}\bigr)
    \]
    defines a t-structure on $\dcohbG X$.
\end{Thm}
