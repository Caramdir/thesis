...
It is also equivalent to the full subcategory of $\dqcbG{X}$ consisting of complexes with coherent cohomology \cite[?]{ArinkinBezrukavnikov:2010:PerverseCoherentSheaves}\todo{Look up reference.}.
...
\todo{All the theorems from \cite{BrodmannSharp:1998:LocalCohomology}, \cite{SGA2} and \cite{Hartshorne:1966:ResiduesAndDuality} that we need.}
\todo{Connecting text.}

\begin{Lem}
    \label{lem:pre:stalk-and-costalk-vanishing}%
    Let $\sheaf F$ be a coherent sheaf on $X$ and let $x$ be a closed point of $X$.
    Then $\mathbf ι_z^* \dualize \sheaf F ∈ \dcat{\ge 0}{\O_x}$ if and only if $\mathbf ι_z^! \sheaf F ∈ \dcat{\le 0}{\O_x}$.
\end{Lem}
...
\todo{Middle perversity. Example. Achar's notes.}
