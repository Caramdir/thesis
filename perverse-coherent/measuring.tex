...
\begin{Assumption}
    We will always assume that $p$ is a monotone and comonotone perversity function.
...
\begin{Rem}
    ...
    We will show in Theorem~\ref{thm:existence_of_lcimeasuring} that the condition $-\dim x \le p(x) \le 0$ is sufficient for the (local) existence of a \lciname\ $p$-measuring collection.
\end{Rem}
...
We now obtain the second part of the theorem by a duality argument from the first one.
