...
Let $\stack X$ be any stack (recall the assumptions we made in Chapter~\ref{ch:d-mod:pre}).
...
\section{Relative compactification for quotient stack}
...
\begin{Def}
    A quotient stack $\stack X = X/G$ is called \emph{\goodstack} if for every quotient stack $\stack Y = Y/G$ and morphism $\stack Y → \stack X$ one has $i_{\stack Y}^* \bar q_{\stack Y}^! j_! = 0$.
\end{Def}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        0.
    \end{equation}
    The second statement follows form the fact that vanishing of a sheaf can be tested locally and the same argument as~\eqref{eq:lem:d-mod:cover}.
\end{proof}

\todo{Similarly: smooth covers}
